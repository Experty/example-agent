...
const analyzeMarketSentiment = async (symbol: string): Promise<MarketSentimentResponse> => {
  try {
    const response = await fetch('https://api.alternative.me/fng/');
    
    if (!response.ok) {
      throw new Error(`Failed to fetch market sentiment data: ${response.statusText}`);
    }
...
export const marketSentiment = createTool({
  ...
  inputSchema: z.object({
    symbol: z.string().describe("The cryptocurrency symbol to analyze sentiment for (e.g., bitcoin, ethereum)")
  }),
  description: "Analyzes the current market sentiment for a cryptocurrency using the Fear & Greed Index",
